...
class ManhattanMaxHeuristic(Heuristic):
    ...
    def __call__(self, state):
        ...
        dist = 0
        for (a, b) in coins:
            if not state.has_collected_coin_at_location(a, b):
                dist = max(dist, abs(x - a) + abs(y - b))
        return dist
...
class ManhattanOrderedSumHeuristic(Heuristic):
    ...
    def __call__(self, state):
        ...
        for i in range(len(coins)):
            ...
            next_x, next_y = visited[-1]
            for (a, b) in coins:
                if (not state.has_collected_coin_at_location(a, b)) and (not (a, b) in visited):
                    if abs(last_x - a) + abs(last_y - b) < dist:
                        next_x, next_y = (a, b)
                        dist = abs(last_x - a) + abs(last_y - b)
